...
import { Clock, Plus, Pencil, Download, Archive, RotateCcw } from "lucide-react";
import { Skeleton } from "@/components/ui";
import { formatDateTime } from "@/lib/format";
import type { TenantPermissionActivity } from "../types";

interface TenantPermissionActivityTabProps {
  activities?: TenantPermissionActivity[];
  loading?: boolean;
}
...
const ACTION_COLORS: Record<string, string> = {
  ...
  updated: "text-blue-600 bg-blue-100 dark:bg-blue-900/20",
...
function TenantPermissionActivityTab({ activities, loading }: TenantPermissionActivityTabProps) {
  if (loading) {
    return (
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">لا توجد نشاطات مسجلة لهذه الصلاحية</p>
      </div>
    );
  }
...
export { TenantPermissionActivityTab };
