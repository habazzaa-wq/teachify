...
export const platformDomainsService = {
  ...
  async getById(id: string): Promise<PlatformDomain | null> {
    try {
      const { data } = await platformApi.get(`/domains/${id}`);
  ...
  async create(data: CreateDomainPayload): Promise<PlatformDomain> {
    const { data: result } = await platformApi.post("/domains", {
  ...
  async update(id: string, data: Partial<PlatformDomain>): Promise<PlatformDomain | null> {
    try {
      ...
      const { data: result } = await platformApi.put(`/domains/${id}`, payload);
  ...
  async delete(id: string): Promise<void> {
    await platformApi.delete(`/domains/${id}`);
  },
  ...
  async bulkMakePrimary(ids: string[]): Promise<void> {
    if (ids.length > 0) {
      await platformApi.put(`/domains/${ids[0]}`, { is_primary: true });
    }
  },

  async renewSsl(id: string): Promise<PlatformDomain | null> {
    const { data: result } = await platformApi.get(`/domains/${id}`);
    return result.domain ? mapDomain(result.domain) : null;
  },
