"use client";

import { Search, RefreshCw, ArrowUpDown } from "lucide-react";
import { AppButton, AppInput, AppSelect, AppSelectTrigger, AppSelectValue, AppSelectContent, AppSelectItem } from "@/components/ui";
import { STATUS_OPTIONS, SYSTEM_ROLE_OPTIONS, DEFAULT_ROLE_OPTIONS, USERS_COUNT_OPTIONS, DATE_CREATED_OPTIONS, DATE_UPDATED_OPTIONS, SORT_OPTIONS } from "../constants";
import type { RoleStatus } from "../types";

interface TenantRolesToolbarProps {
  search: string;
  onSearchChange: (value: string) => void;
  statusFilter: RoleStatus | "all";
  onStatusChange: (value: RoleStatus | "all") => void;
  systemFilter: string;
  onSystemChange: (value: string) => void;
  defaultFilter: string;
  onDefaultChange: (value: string) => void;
  usersCountFilter: string;
  onUsersCountChange: (value: string) => void;
  dateCreatedFilter: string;
  onDateCreatedChange: (value: string) => void;
  dateUpdatedFilter: string;
  onDateUpdatedChange: (value: string) => void;
  sort: string;
  onSortChange: (value: string) => void;
  onRefresh: () => void;
  refreshing?: boolean;
}

function TenantRolesToolbar({
  search,
  onSearchChange,
  statusFilter,
  onStatusChange,
  systemFilter,
  onSystemChange,
  defaultFilter,
  onDefaultChange,
  usersCountFilter,
  onUsersCountChange,
  dateCreatedFilter,
  onDateCreatedChange,
  dateUpdatedFilter,
  onDateUpdatedChange,
  sort,
  onSortChange,
  onRefresh,
  refreshing,
}: TenantRolesToolbarProps) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      <div className="relative flex-1 min-w-[200px] max-w-sm">
        <Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <AppInput
          value={search}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder="بحث بالاسم أو الوصف..."
          className="ps-9 h-9"
        />
      </div>

      <AppSelect
        value={statusFilter}
        onValueChange={(val) => onStatusChange(val as RoleStatus | "all")}
      >
        <AppSelectTrigger className="h-9 w-[130px]">
          <AppSelectValue placeholder="الحالة" />
        </AppSelectTrigger>
        <AppSelectContent>
          {STATUS_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect
        value={systemFilter}
        onValueChange={onSystemChange}
      >
        <AppSelectTrigger className="h-9 w-[135px]">
          <AppSelectValue placeholder="أدوار النظام" />
        </AppSelectTrigger>
        <AppSelectContent>
          {SYSTEM_ROLE_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect
        value={defaultFilter}
        onValueChange={onDefaultChange}
      >
        <AppSelectTrigger className="h-9 w-[135px]">
          <AppSelectValue placeholder="افتراضي" />
        </AppSelectTrigger>
        <AppSelectContent>
          {DEFAULT_ROLE_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect
        value={usersCountFilter}
        onValueChange={onUsersCountChange}
      >
        <AppSelectTrigger className="h-9 w-[135px]">
          <AppSelectValue placeholder="عدد المستخدمين" />
        </AppSelectTrigger>
        <AppSelectContent>
          {USERS_COUNT_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect
        value={dateCreatedFilter}
        onValueChange={onDateCreatedChange}
      >
        <AppSelectTrigger className="h-9 w-[130px]">
          <AppSelectValue placeholder="تاريخ الإنشاء" />
        </AppSelectTrigger>
        <AppSelectContent>
          {DATE_CREATED_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect
        value={dateUpdatedFilter}
        onValueChange={onDateUpdatedChange}
      >
        <AppSelectTrigger className="h-9 w-[130px]">
          <AppSelectValue placeholder="تاريخ التحديث" />
        </AppSelectTrigger>
        <AppSelectContent>
          {DATE_UPDATED_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppSelect value={sort} onValueChange={onSortChange}>
        <AppSelectTrigger className="h-9 w-[130px]">
          <ArrowUpDown className="h-3.5 w-3.5" />
          <AppSelectValue placeholder="ترتيب" />
        </AppSelectTrigger>
        <AppSelectContent>
          {SORT_OPTIONS.map((opt) => (
            <AppSelectItem key={opt.value} value={opt.value}>
              {opt.label}
            </AppSelectItem>
          ))}
        </AppSelectContent>
      </AppSelect>

      <AppButton variant="outline" size="sm" className="h-9" onClick={onRefresh} loading={refreshing}>
        <RefreshCw className="h-4 w-4" />
        تحديث
      </AppButton>
    </div>
  );
}

export { TenantRolesToolbar };
