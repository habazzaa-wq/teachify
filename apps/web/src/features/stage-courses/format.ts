import type { StageAggregates, StageCourse, StageCoursesResponse, StageSubject, StageTeacher } from "./types";

/* eslint-disable @typescript-eslint/no-explicit-any */
type Raw = Record<string, any>;

export function formatStageCourse(raw: Raw): StageCourse {
  return {
    id: String(raw.id),
    title: raw.title,
    slug: raw.slug,
...
function formatTeacher(raw: Raw): StageTeacher {
  return {
    id: String(raw.id),
    name: raw.name ?? "مدرس",
...
function formatSubject(raw: Raw): StageSubject {
  return {
    id: String(raw.id),
    name: raw.name ?? "مادة",
    coursesCount: raw.coursesCount ?? 0,
...
function formatAggregates(raw: Raw | undefined): StageAggregates {
...
export function formatStageCoursesResponse(raw: Raw): StageCoursesResponse {
  return {
    data: (raw.data ?? []).map(formatStageCourse),
    total: raw.total ?? 0,
