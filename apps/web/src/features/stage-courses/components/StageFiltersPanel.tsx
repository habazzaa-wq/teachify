...
import type { StageCourseFilters, StagePricingFilter, StageSort, StageSubject, StageTeacher } from "../types";
import { PRICING_OPTIONS, PRIMARY, SORT_OPTIONS } from "../constants";

interface StageFiltersPanelProps {
  searchDraft: string;
  onSearchDraftChange: (value: string) => void;
  onSearchSubmit: () => void;
  filters: StageCourseFilters;
  onChange: (patch: Partial<StageCourseFilters>) => void;
  onReset: () => void;
...
export function StageFiltersPanel({
  searchDraft,
  onSearchDraftChange,
  onSearchSubmit,
  filters,
    ...
    <div dir="rtl" className="flex flex-col gap-5 p-5">
          ...
          {PRICING_OPTIONS.map(({ value, label }) => {
            ...
            return (
              <button
                key={value}
                type="button"
                onClick={() => onChange({ pricing: value as StagePricingFilter })}
      ...
      <div>
        ...
        <AppSelect
          ...
          onValueChange={(value) => onChange({ sort: value as StageSort })}
        >
          <AppSelectTrigger style={{ background: selectBg }}>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          إعادة تعيين
          {activeCount > 0 && (
            <span className="rounded-full px-1.5 text-[11px] font-bold" style={{ background: `rgb(var(--brand-primary-rgb) / 0.133)`, color: PRIMARY }}>
              {activeCount}
            </span>
          )}
        </AppButton>
