...
import type { StageCourse } from "../types";
import { PRIMARY } from "../constants";
import { StageCourseCard } from "./StageCourseCard";

interface StageCourseGridProps {
  courses?: StageCourse[];
  isLoading: boolean;
  total: number;
  currentPage: number;
...
export function StageCourseGrid({
  courses,
  isLoading,
  total,
  currentPage,
  ...
  if (!isLoading && courses && courses.length === 0) {
    return (
      <AppEmptyState
        ...
        description="جرّب تعديل الفلاتر أو البحث بكلمات أخرى للعثور على دورات في هذه المرحلة."
        action={
          <Link
            href="/"
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
        {(courses ?? []).map((course, i) => (
          <StageCourseCard key={course.id} course={course} index={i} />
        ))}
      </div>
