...
export function useSuspendStudent() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => studentsService.suspend(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [STUDENTS_QUERY_KEY] });
    },
  });
...
export function useBulkActivateStudents() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (ids: string[]) => studentsService.bulkActivate(ids),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [STUDENTS_QUERY_KEY] });
    },
  });
...
export function useBulkSuspendStudents() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (ids: string[]) => studentsService.bulkSuspend(ids),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [STUDENTS_QUERY_KEY] });
    },
  });
