 ...
 * `tone` adapts link / inline-code colors to the bubble background:
 * "primary" renders inside the sender's primary-colored bubble.
 */
export function MessageContent({
  ...
  return (
    <div dir="auto" className={cn("text-sm leading-relaxed", className)}>
      <ReactMarkdown
        remarkPlugins={[remarkGfm, remarkMath]}
        rehypePlugins={[rehypeKatex]}
        components={buildMarkdownComponents(tone)}
      >
        {body}
      </ReactMarkdown>
    </div>
...
function buildMarkdownComponents(tone: "primary" | "secondary") {
  ...
  return {
    ...
    blockquote: ({ children }: { children?: React.ReactNode }) => (
      <blockquote
        className={
          primary
            ? "my-1 border-s-2 border-white/50 ps-3 text-white/90"
            : "my-1 border-s-2 border-primary/40 ps-3 text-muted-foreground"
        }
      >
        {children}
      </blockquote>
    ...
    code: ({ className, children }: { className?: string; children?: React.ReactNode }) => {
      ...
      return (
        <code
          className={
            primary
              ? "rounded bg-white/15 px-1.5 py-0.5 font-mono text-[0.85em] text-white"
