...
class ResumableUploadTest extends TestCase
{
    ...
    /* ------------------------------------------------------------------ *
     * Regression coverage: resumable reliability + idempotency
     * ------------------------------------------------------------------ */

    private function createTwoChunkSession(array $overrides = []): array
    {
        $tenant = $this->createTenant();
        $this->createBunnyStorageIntegration($tenant);
        $member = $this->memberWithRole($tenant, 'tenant_owner');
        ...
        $sessionId = $this->postJson('/api/v1/media-library/upload/resumable/intent', array_merge([
            'type' => 'file',
            'original_filename' => 'sample.bin',
            'mime_type' => 'application/octet-stream',
            'size_bytes' => 2500,
            'total_chunks' => 2,
        ], $overrides), $this->tenantHeader($tenant))->json('data.session_id');

        $session = MediaUploadSession::findOrFail($sessionId);

        return compact('tenant', 'member', 'content', 'chunk0', 'chunk1', 'hash0', 'hash1', 'fileHash', 'sessionId', 'session');
    }

    /** Retrying a chunk that was already uploaded is safe and idempotent. */
    public function test_duplicate_chunk_is_idempotent_and_finalize_succeeds(): void
    {
        Storage::fake('uploads');

        $f = $this->createTwoChunkSession();

        // Upload both chunks, then re-send chunk 0 (simulating a client retry).
        $this->putChunk($f['tenant'], $f['session'], 0, $f['chunk0'], $f['hash0'])->assertOk();
        $this->putChunk($f['tenant'], $f['session'], 1, $f['chunk1'], $f['hash1'], 1500)->assertOk();
        $this->putChunk($f['tenant'], $f['session'], 0, $f['chunk0'], $f['hash0'])->assertOk();

        // Exactly one DB row per chunk index (no duplication/corruption).
        $this->assertSame(
            1,
            MediaUploadChunk::where('media_upload_session_id', $f['sessionId'])
                ->where('chunk_index', 0)
                ->count(),
        );
        ...
        $mock->shouldReceive('pushToBunny')->once()->andReturnUsing(function (MediaUploadSession $s, string $path) {
            $asset = $s->asset;
            $asset->forceFill(['status' => 'ready', 'processing_status' => 'ready'])->save();
            return $asset;
        });
        app()->instance(ResumableUploadService::class, $mock);

        $this->postJson(
            "/api/v1/media-library/upload/resumable/{$f['sessionId']}/finalize",
            ['file_hash' => $f['fileHash']],
            $this->tenantHeader($f['tenant']),
        )->assertOk();
    }

    /** An incomplete upload (missing chunk) can never become ready. */
    public function test_incomplete_upload_cannot_be_finalized(): void
    {
        Storage::fake('uploads');
        ...
        $this->assertDatabaseHas('media_upload_sessions', ['id' => $f['sessionId'], 'completed' => false]);
        $asset = MediaAsset::findOrFail($f['session']->media_asset_id);
        $this->assertNotSame('ready', $asset->status);
    }

    /** A final byte-count mismatch on assembly is rejected (size not checked). */
    public function test_finalize_rejects_when_declared_size_does_not_match_assembly(): void
    {
        Storage::fake('uploads');

        // Declare a size larger than the actual chunked body we upload.
        $f = $this->createTwoChunkSession(['size_bytes' => 9999]);

        // Content-Range total must match the session size, so send a full-range header.
        $this->putChunk($f['tenant'], $f['session'], 0, $f['chunk0'], $f['hash0'])->assertOk();
        $this->putChunk($f['tenant'], $f['session'], 1, $f['chunk1'], $f['hash1'], 1500)->assertOk();
        ...
        // 2500 assembled != 9999 declared -> rejected; never becomes ready.
        $resp->assertStatus(422);
        $asset = MediaAsset::findOrFail($f['session']->media_asset_id);
        $this->assertNotSame('ready', $asset->status);
    }

    /** Finalize is idempotent and creates exactly one MediaAsset. */
    public function test_finalize_runs_once_and_creates_exactly_one_asset(): void
    {
        Storage::fake('uploads');
        ...
        $url = "/api/v1/media-library/upload/resumable/{$f['sessionId']}/finalize";
        $this->postJson($url, ['file_hash' => $f['fileHash']], $this->tenantHeader($f['tenant']))->assertOk();

        // A second finalize returns 200 and does not create a second asset.
        $this->postJson($url, ['file_hash' => $f['fileHash']], $this->tenantHeader($f['tenant']))->assertOk();

        $this->assertSame(
            1,
            MediaAsset::where('tenant_id', $f['tenant']->id)
                ->where('original_filename', 'sample.bin')
                ->count(),
        );
    }

    /** A large multi-chunk upload (small-video surrogate) finalizes cleanly. */
    public function test_many_chunk_upload_succeeds_small_video_surrogate(): void
    {
        Storage::fake('uploads');
        ...
        $chunkCount = 60;
        $chunkSize = 512;
        $content = random_bytes($chunkCount * $chunkSize);
        $hashes = [];
        ...
        for ($i = 0; $i < $chunkCount; $i++) {
            ...
            $hashes[$i] = $hash;
            $this->putChunk($tenant, $session, $i, $bytes, $hash, $i * $chunkSize)->assertOk();
        }

        // Resume reports every chunk uploaded.
        $resume = $this->getJson(
            "/api/v1/media-library/upload/resumable/{$sessionId}/resume",
            $this->tenantHeader($tenant),
        );
        $resume->assertOk();
        $resume->assertJsonCount($chunkCount, 'data.completed_chunks');
        $resume->assertJsonPath('data.remaining_chunks', []);

        $fileHash = $this->combinedHash($hashes);
    ...
    /** A session created by one tenant cannot be written to by another. */
    public function test_tenant_isolation_for_chunk_upload(): void
    {
        Storage::fake('uploads');
        ...
        $chunk = random_bytes(1500);
        $hash = hash('sha256', $chunk);

        $sessionId = $this->postJson('/api/v1/media-library/upload/resumable/intent', [
            'type' => 'file',
            'original_filename' => 'iso.bin',
            'mime_type' => 'application/octet-stream',
            'size_bytes' => 2500,
            'total_chunks' => 2,
        ], $this->tenantHeader($tenant))->json('data.session_id');
        ...
        // Another tenant attempting to write to this session is rejected.
        $otherTenant = $this->createTenant();
        $this->putChunk($otherTenant, $session, 0, $chunk, $hash)->assertStatus(404);

        // No chunk row was created for the foreign write.
        $this->assertSame(
            0,
            MediaUploadChunk::where('media_upload_session_id', $sessionId)->count(),
        );
    }
}
